...
class SegmentationPredictor(DetectionPredictor):
    ...
    def write_results(self, idx, results, batch):
        ...
        det, mask = (
            result.boxes,
            result.masks,
        )  # getting tensors TODO: mask mask,box inherit for tensor
        ...
        if self.args.save or self.args.show:
            im_gpu = (
                torch.as_tensor(im0, dtype=torch.float16, device=mask.masks.device)
                .permute(2, 0, 1)
                .flip(0)
                .contiguous()
                / 255
        ...
        for j, d in enumerate(reversed(det)):
            ...
            if self.args.save_txt:  # Write to file
                seg = (
                    mask.xyn[len(det) - j - 1].copy().reshape(-1)
                )  # reversed mask.xyn, (n,2) to (n*2)
                line = (
                    (c, *seg)
                    + (conf,) * self.args.save_conf
